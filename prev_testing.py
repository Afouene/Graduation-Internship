...
from prev_env import AUVEnvironment  
import numpy as np
import matplotlib.pyplot as plt
import csv
#model_path="logs/53/rl_model_4860000_steps.zip" #7 nodes

#model_path="logs/51/rl_model_9750000_steps.zip" # 10 nodes
model_path="./rl_model_2620000_steps.zip" # 3 nodes
#model_path="./rl_model_2800000_steps.zip" # 3 nodes
...
with open('auv_positions3nodes3d.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerows(auv_positions)
    
np.savetxt('sensor_nodes3nodes.csv', env.sensor_node_positions, delimiter=',')
...
print(" total nbr of communication with",env.num_devices,"nodes ",np.sum(env.occurence))
